...
const Button: React.FC<ButtonProps> = ({
  ...
  const variants = {
    primary: "bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-200 border-none",
    ...
    white: "bg-white text-slate-900 hover:bg-slate-50 shadow-2xl shadow-black/5 border-none"
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="bg-indigo-600 p-2 rounded-lg"><BrainCircuit className="h-6 w-6 text-white" /></div>
            <span className="text-2xl font-black text-slate-900 tracking-tighter">Plania</span>
          </div>
          <nav className="hidden lg:flex space-x-8 items-center">
            {['Funcionalidades', 'Demonstração', 'Depoimentos', 'Preços'].map((item) => (
              <a key={item} href={`#${item.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")}`} className="text-slate-600 hover:text-indigo-600 font-bold transition-colors text-sm uppercase tracking-widest">{item}</a>
            ))}
            <div className="h-6 w-px bg-slate-200"></div>
            <button onClick={() => onNavigate('login')} className="text-indigo-600 font-black hover:text-indigo-800 transition-colors text-sm uppercase tracking-widest">Entrar</button>
      ...
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-32 lg:pt-56 lg:pb-40 overflow-hidden">
        <div className="absolute top-0 right-0 -mr-32 -mt-32 w-[600px] h-[600px] bg-indigo-200/40 rounded-full blur-[120px] pointer-events-none"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div className="text-center lg:text-left">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-indigo-50 border border-indigo-100 mb-10 shadow-sm animate-fade-in">
                <Sparkles className="h-4 w-4 text-indigo-600 mr-2" />
                <span className="text-[10px] font-black text-indigo-700 tracking-[0.2em] uppercase">IA Pedagógica Brasileira</span>
              </div>
              <h1 className="text-6xl md:text-7xl lg:text-[90px] xl:text-[100px] font-black text-slate-900 tracking-tighter mb-8 leading-[0.9] lg:leading-[0.85]">
                O professor <br /> agora tem <br /> <span className="gradient-text">superpoderes.</span>
              </h1>
              <p className="text-xl md:text-2xl text-slate-500 mb-12 leading-relaxed max-w-xl mx-auto lg:mx-0 font-medium">Esqueça a burocracia. O Plania usa IA de ponta para gerar planos de aula BNCC e diários de classe em segundos.</p>
            ...
            <div className="relative group animate-float hidden lg:block">
              ...
              <div className="relative bg-white rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] border border-slate-100 p-10 z-20 transform hover:-rotate-1 transition-transform duration-500">
                <div className="flex items-center space-x-4 mb-10 pb-6 border-b border-slate-50">
                  ...
                  <div><p className="text-lg font-black text-slate-900">Assistente Plania</p><p className="text-xs text-emerald-500 font-bold flex items-center"><span className="w-2 h-2 bg-emerald-500 rounded-full mr-1.5 animate-pulse"></span> Sistema Pronto</p></div>
                </div>
                <div className="space-y-6">
                  <div className="flex justify-end"><div className="bg-indigo-600 text-white rounded-3xl rounded-tr-sm px-6 py-4 text-sm font-bold shadow-xl">Crie um plano sobre Fotossíntese para o 6º ano.</div></div>
                  <div className="bg-slate-50 rounded-[2rem] p-8 border border-slate-100"><div className="flex gap-2 mb-5"><span className="text-[10px] bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full font-black uppercase">BNCC EF06CI07</span></div><h3 className="font-black text-slate-900 mb-4 text-xl tracking-tight">Energia Solar e Vida</h3><div className="space-y-3"><div className="h-2.5 bg-slate-200 rounded-full w-3/4"></div><div className="h-2.5 bg-slate-200 rounded-full w-full"></div></div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="funcionalidades" className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-28">
            <span className="text-xs font-black text-indigo-600 uppercase tracking-[0.4em] mb-6 block">Infraestrutura Escolar</span>
            <h2 className="text-5xl md:text-7xl font-black text-slate-900 mb-8 tracking-tighter">Ferramentas que <br /> <span className="gradient-text">libertam</span> você.</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
            {[
              ...
              <div key={i} className="group p-10 rounded-[3rem] bg-[#f8fafc] hover:bg-white border border-transparent hover:border-slate-100 transition-all duration-500 hover:shadow-2xl">
                <div className={`bg-gradient-to-br ${f.color} w-20 h-20 rounded-[1.75rem] flex items-center justify-center mb-10 text-white shadow-lg group-hover:scale-110 transition-transform`}>{f.icon}</div>
      ...
      {/* DEMO SECTION */}
      <section id="demonstracao" className="py-32 bg-slate-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            <h2 className="text-5xl font-black text-slate-900 mb-6 tracking-tight">Experimente a <span className="gradient-text">inteligência</span>.</h2>
            <p className="text-xl text-slate-500 font-medium">Escolha um tema e veja a IA criar seu plano profissional.</p>
          </div>
          <div className="grid lg:grid-cols-12 gap-12 items-start">
              ...
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl">
                <form onSubmit={handleGenerateDemo} className="space-y-8">
                  <div>
                    ...
                    <select value={grade} onChange={(e) => setGrade(e.target.value as GradeLevel)} className="w-full px-5 py-4 rounded-2xl border border-slate-100 bg-slate-50 font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/10 appearance-none">
                  ...
                  <div>
                    ...
                    <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Ex: Sistemas Solares" className="w-full px-5 py-4 rounded-2xl border border-slate-100 bg-slate-50 font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/10" />
                  </div>
            ...
            <div className="lg:col-span-7">
              <div className="relative min-h-[600px] bg-white rounded-[2rem] border border-slate-200 shadow-2xl overflow-hidden flex flex-col">
                {isLoadingDemo ? (
                  ...
                  <div className="p-10 lg:p-14 animate-fade-in overflow-y-auto max-h-[600px]">
                    <span className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg text-[10px] font-black uppercase mb-6 inline-block">{grade}</span>
                    <h3 className="text-4xl font-black text-slate-900 mb-10 tracking-tight">{plan.title}</h3>
                    <div className="grid sm:grid-cols-2 gap-8 mb-12">
                      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100"><span className="text-indigo-600 text-xs font-black uppercase flex items-center mb-2"><Clock className="w-4 h-4 mr-2" /> Duração</span><p className="font-bold text-slate-800">{plan.duration}</p></div>
                      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100"><span className="text-indigo-600 text-xs font-black uppercase flex items-center mb-2"><BookOpen className="w-4 h-4 mr-2" /> BNCC</span><div className="flex flex-wrap gap-2">{plan.bnccCodes.map(c => <span key={c} className="bg-white px-2 py-1 rounded-lg text-[10px] font-bold border border-indigo-100">{c}</span>)}</div></div>
                    </div>
                    <div className="space-y-12">
                      <div><h4 className="text-xl font-black mb-6 flex items-center"><Target className="w-6 h-6 mr-3 text-indigo-500" /> Objetivos</h4><ul className="space-y-4">{plan.objectives.map((obj, i) => <li key={i} className="flex items-start text-slate-600 font-medium"><div className="mr-4 mt-2 w-2 h-2 bg-indigo-500 rounded-full flex-shrink-0" />{obj}</li>)}</ul></div>
                      <div><h4 className="text-xl font-black mb-6 flex items-center"><Sparkles className="w-6 h-6 mr-3 text-indigo-500" /> Metodologia</h4><p className="text-slate-600 font-medium leading-relaxed">{plan.methodology}</p></div>
                    </div>
                  </div>
                ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
            ...
            <div className="relative group">
              ...
              <div className="relative bg-indigo-600 p-12 rounded-[3rem] text-white flex flex-col h-full overflow-hidden shadow-2xl">
